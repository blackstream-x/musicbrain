...
def fix_file_vbr_length(file_path, target_directory_path):
    ...
    if fixed_file_digest == original_digest:
        ...
        LOGGER.info("File_size: %s bytes", os.stat(output_file_path).st_size)
        LOGGER.debug("… checksum: %s", fixed_file_digest)
    else:
        LOGGER.heading("File changed")
        LOGGER.info("Old file size: %s bytes", os.stat(file_path).st_size)
        LOGGER.debug("… checksum: %s", original_digest)
        LOGGER.info(
...
def fix_all_files(input_directory, output_directory):
    ...
    for file_path in sorted(input_directory.glob("*")):
        if not file_path.is_file():
            LOGGER.debug("Skipped %r: not a regular file", file_path.name)
            continue
        #
        mime_type = mimetypes.guess_type(file_path)[0]
        if not mime_type or not mime_type.startswith("audio/"):
            LOGGER.debug("Skipped file %r: no audio mime type", file_path.name)
            continue
        #
    ...
    #
    LOGGER.separator()
    LOGGER.info("%s files processed, %s errors", len(processed_files), errors)
    if errors:
        return RETURNCODE_ERROR
    #
...
def __get_arguments():
    ...
    argument_parser.set_defaults(
        loglevel=dialog.logging.INFO, input_directory=pathlib.Path.cwd()
    )
    ...
    if arguments.output_directory:
        if arguments.output_directory.is_absolute():
            ...
            output_directory = pathlib.Path(
                os.path.normpath(input_directory / arguments.output_directory)
            )
        #
    else:
